...
export default function Notifications() {
  ...
  // Bug 4 fix: announcements track read state in localStorage
  ...
  useEffect(() => {
    ...
    const subscription = supabase.channel('teacher:notifications:v2')
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'notifications'
      ...
      // Bug 15-teacher: also listen to announcements table so new broadcasts appear instantly
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'announcements'
  ...
  const markAsRead = async (id) => {
    ...
    if (notif.is_broadcast) {
      // Bug 4 fix: persist in localStorage, not DB
      const readSet = getReadAnnouncements();
      readSet.add(id);
  ...
  // Bug 5 fix: markAllAsRead now clears BOTH notification rows AND announcement-sourced items
