...
export default function AITutorDrawer({ isOpen, onClose, botState, setBotState, onNewMessage }) {
  ...
  const voiceResult = useCallback((finalText) => {
    if (finalText.trim() && sendRef.current) sendRef.current(finalText);
  }, []);

  /* After speaking in voice mode, auto-listen again */
  const afterSpeak = useCallback(() => {
    if (!voiceModeRef.current) return;
    setTimeout(() => { if (voiceModeRef.current) startListening(voiceResult); }, 500);
  }, [startListening, voiceResult]);
  ...
  const toggleVoiceMode = useCallback(() => {
    ...
    else           { setVoiceMode(true);  startListening(voiceResult); }
  }, [voiceMode, stopListening, stopSpeaking, startListening, voiceResult]);
