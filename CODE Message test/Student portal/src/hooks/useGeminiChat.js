...
function loadSavedMessages() {
  try {
    const raw = localStorage.getItem("aria_chat_history");
...
/* ── Fast page context (targeted elements, no body.innerText freeze) ── */
...
export default function useGeminiChat() {
  ...
  // Sync initial historyRef from saved messages
  useEffect(() => {
    const saved = loadSavedMessages();
    historyRef.current = saved.map(m => ({ role: m.role, parts: [{ text: m.text }] })).slice(-MAX_HISTORY);
  }, []);

  const updateAndSaveMessages = useCallback((updater) => {
    setMessages(prev => {
    ...
    try {
      ...
      while (true) {
        ...
        while ((nl = buf.indexOf("\n")) !== -1) {
          ...
          if (tok) {
            accumulated += tok;
            updateAndSaveMessages(prev => prev.map(m => m.id === streamId ? { ...m, text: accumulated } : m));
          }
        }
      }

      updateAndSaveMessages(prev => prev.map(m => m.id === streamId ? { ...m, streaming: false } : m));
      ...
      setError(msg);
      updateAndSaveMessages(prev => prev.map(m => m.id === streamId ? { ...m, text: msg, streaming: false } : m));
      return null;
    }
