...
export default function ChatInterface({ currentUser: propUser, activeTab, selectedClass, isManager, onUnreadCountChange }) {
  ...
  useEffect(() => {
    ...
    const profilesSub = supabase.channel('public:profiles_sync_student')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, fetchProfilesAndConversations)
      .subscribe();
      
    const groupsSub = supabase.channel('public:conversations:groups_sync_student')
  ...
  useEffect(() => {
    fetchUnreadCounts();
    const unreadSub = supabase.channel('public:messages:unread_sync_student')
          ...
          <>
            <div className="chat-main-header">
              ...
              <div className="chat-header-actions" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <button 
                  ...
                  style={{ background: showInChatSearch ? 'rgba(0, 229, 255, 0.2)' : 'transparent' }}
                >
                  <SearchIcon size={18} />
                </button>
              </div>
                    ...
                    >
                      ...
                      <div className="chat-bubble">
                        ...
                        <div className="chat-bubble-time">
                          ...
                          {isMe && !msg.is_deleted && (
                            msg.is_read ? (
                              <CheckCheck size={13} style={{ marginLeft: '4px', verticalAlign: 'middle', color: 'var(--accent-cyan, #00E5FF)' }} title="Read" />
                            ) : (
            ...
            <form className="chat-input-area" onSubmit={handleSendMessage} style={{ position: 'relative' }}>
              ...
              <button 
                ...
                title="Insert Emoji"
                style={{ color: showEmojiPicker ? 'var(--accent-cyan, #00E5FF)' : 'inherit' }}
              >
                <Smile size={20} />
              </button>
